...
; revision history:
;
;edit 35, 21-Jun-2009 by RK. added code for the KC(85)Net-Interface 
;	fore using CP/M KERMIT over TCP/IP
...
cpsker	EQU	FALSE		; building the system-independent part...
...
cpm3	EQU	FALSE		;"Generic" Kermit-80 for CP/M 3.0 (CP/M Plus)
				; (terminal required)
kc85n	EQU	TRUE		;KC(85)Net CP/M 2.x system with TCP/IP interface
...
IF kc85n
cpuspd	SET	33		; KC85 has 4 MHz Z80 but CPC has a reduced clock ~3.3MHz due to CRTC wait states
ENDIF; kc85n
...
k85fam	SET	FALSE		; not a KC85 system
...
IF kc85n
k85fam	SET	TRUE		; doing a KC(85)Net system
.printx * k85fam set TRUE *
ENDIF	; kc85n
...
IF (genfam OR trsfam OR z80fam OR beefam OR k85fam) AND sysfam
sysfam	SET	FALSE		; Were not doing the CPXSYS.ASM file
.printx * sysfam set FALSE *
ENDIF	; (genfam OR trsfam OR z80fam OR mbeefam OR k85fam) AND sysfam
...
;
	INCLUDE CPXK85.ASM	;system dependent KERMIT file
;
	.8080			;z80 OFF
ENDIF	;k85fam
